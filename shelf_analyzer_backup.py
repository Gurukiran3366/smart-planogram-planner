...
def build_expected_lookup(expected: Dict[str, Any]) -> Dict[int, List[Dict[str, Any]]]:
...
def evaluate_colour_sequence(
    ...
    sequence = rule.get("colour_sequence", [])
...
def evaluate_missing_and_occupied_targets(
    ...
    for expected in expected_products:
        ...
        occupant = actual_occupancy.get(slot) if slot is not None else None
        ...
        missing.append({
            ...
            "destination_occupant_name": (
                product_name(catalog, occupant) if occupant else None
            ),
            "diagnosis": (
...
def analyze_rack(
    ...
    expected_lookup = build_expected_lookup(expected)
...
def main() -> None:
    ...
    result = analyze_rack(actual, expected, rules, catalog)
